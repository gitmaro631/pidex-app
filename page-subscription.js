...
const FEATURES = [
  ...
  { icon: '👛',
    ...
    desc_ko: 'Pi 로그인 시 지갑 주소 자동 연결을 시도하며, 연결되지 않을 경우 직접 입력해 사용할 수 있습니다. Pi 잔액·토큰·LP 예치 현황을 조회합니다.',
    desc_en: 'Attempts to auto-connect your wallet on Pi login. If not detected, enter your address manually to view Pi balance, tokens and LP positions.',
    desc_id: 'Mencoba menghubungkan dompet secara otomatis saat login Pi. Jika gagal, masukkan alamat secara manual untuk melihat saldo, token, dan posisi LP.',
    desc_zh: 'Pi登录时尝试自动连接钱包，若未检测到可手动输入地址，查看Pi余额、代币及LP质押状况。',
    desc_ja: 'Piログイン時にウォレットの自動接続を試みます。検出できない場合は手動入力でPi残高・トークン・LP預入状況を確認できます。',
    desc_es: 'Intenta conectar tu cartera automáticamente al iniciar sesión en Pi. Si no se detecta, ingrésala manualmente para ver saldo, tokens y posiciones LP.',
    desc_vi: 'Thử tự động kết nối ví khi đăng nhập Pi. Nếu không phát hiện, nhập thủ công để xem số dư, token và vị thế LP.',
    desc_hi: 'Pi लॉगिन पर वॉलेट ऑटो-कनेक्ट का प्रयास करता है। न मिले तो मैन्युअल दर्ज करें — Pi बैलेंस, टोकन, LP पोजीशन देखें।',
    desc_pt: 'Tenta conectar sua carteira automaticamente no login Pi. Se não detectado, insira manualmente para ver saldo, tokens e posições LP.',
    desc_tl: 'Sinusubukang auto-connect ang wallet sa Pi login. Kung hindi ma-detect, ilagay nang manu-mano para makita ang balanse, tokens at LP positions.',
    desc_fr: 'Tente de connecter votre portefeuille automatiquement à la connexion Pi. Sinon, saisissez-le manuellement pour voir solde, tokens et positions LP.' },
  { icon: '↓',
