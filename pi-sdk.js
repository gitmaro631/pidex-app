...
// Firebase 로그인 실패는 조용히 넘어가지 않고 화면에 표시 — 원인 파악용
function showAuthToast(msg) {
  try {
    const el = document.createElement('div');
    el.textContent = msg;
    el.style.cssText = `position:fixed;left:50%;bottom:24px;transform:translateX(-50%);z-index:99999;padding:10px 16px;border-radius:8px;font-size:13px;color:#fff;background:#ef4444;box-shadow:0 2px 8px rgba(0,0,0,0.3);max-width:90vw;text-align:center;`;
...
async function signInFirebase(accessToken) {
  try {
    if (typeof firebase === 'undefined') { showAuthToast('🔴 Firebase SDK 없음'); return; }
    ...
    if (!res.ok) { showAuthToast(`🔴 Firebase 토큰 발급 실패 (${res.status})`); return; }
    const { token } = await res.json();
    if (!token) { showAuthToast('🔴 Firebase 토큰 없음'); return; }
    await firebase.auth().signInWithCustomToken(token);
  } catch (e) {
    showAuthToast(`🔴 Firebase 로그인 오류: ${e?.message || e}`);
  }
}
