...
async function loadAndRenderAdminStats(el) {
  ...
  try {
    ...
    el.innerHTML = `
      ...
      ${row('구독자 수 (파이덱스 앱)', subscriberCount ?? '?', null)}
    `;
  } catch (e) {
...
function _showNoticePopup(notices, idx) {
  ...
  const isAdmin = getLoggedInUsername() === ADMIN_USERNAME;
